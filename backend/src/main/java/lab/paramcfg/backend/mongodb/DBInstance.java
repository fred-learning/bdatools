...
public class DBInstance {
  ...
  public DBInstance(String host, String dbName, String cName) {
    ...
    setMongoCollection(mongoClient.getDatabase(dbName).getCollection(cName));
  }
  ...
  public JobData getJobData(String jobid) 
    ...
    JobResource resource = Util.deserializeFromMongo(doc.get("limited_resource"), JobResource.class);
    Date startTime = doc.getDate("startTime");
    Date endTime = doc.getDate("endTime");
    JournalData jData = Util.deserializeFromMongo(doc.get("journal_data"), JournalData.class);
    MonitoringData mData = Util.deserializeFromMongo(doc.get("monitoring_data"), MonitoringData.class);
    RDDSData rData = Util.deserializeFromMongo(doc.get("rdds_data"), RDDSData.class); 

    return new JobData(id, status, config, resource, 
                       startTime, endTime,
                       jData, mData, rData);
  } 
  
  public void saveJobData(String id, int status, JobConfig config, 
      JobResource limitedResource,
    ...
    Document doc = new Document().append("id", id)
                                 ...
                                 .append("config", Util.serializeToMongo(config, JobConfig.class))
                                 .append("limited_resource", Util.serializeToMongo(limitedResource, JobResource.class))
                                 .append("startTime", startTime)
                                 .append("endTime", endTime)
                                 .append("journal_data", Util.serializeToMongo(journalData, JournalData.class))
                                 .append("monitoring_data", Util.serializeToMongo(monitoringData, MonitoringData.class))
                                 .append("rdds_data", Util.serializeToMongo(rddsData, RDDSData.class));
